...
def main():
    ...
    if x and camera_image is not None:
        print(f"Original camera image shape: {camera_image.shape}")
        
        # Crop the bottom 20 pixels from the image
        height = camera_image.shape[0]
        cropped_image = camera_image[0:height-40, :]  # Remove bottom 20 pixels
        
        print(f"Cropped camera image shape: {cropped_image.shape}")
        cv2.imshow('QCar RGB Camera (Cropped)', cropped_image)
