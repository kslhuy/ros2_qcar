...
class CommHandler:
    ...
    def __init__(self, vehicle_id: int, target_ip: str, send_port: int, recv_port: int, ack_port: int, logger: logging.LoggerAdapter, running_flag,vehicle):
    ...
    def send_state(self):
        try:
            while self.running.is_set():
                print("send_state active")
                print("current seq:", self.sequence_number)
                ...
                while retries < 3 and not ack_received:
                    try:
                        with self.lock:
                        ...
                        try:
                            ...
                            if (ack.get('type') == 'ack' and 
                                ...
                                with self.lock:
                                    self.sequence_number += 1
                                    print("Incremented sequence_number to", self.sequence_number)
                                self.logger.info(f"ACK received for seq: {self.sequence_number - 1} from {addr}")
                            else:
                                self.logger.debug(f"ACK discarded: {ack}, Expected {self.sequence_number}")
                        except socket.timeout:
                            retries += 1
                            self.logger.warning(f"ACK timeout for seq: {self.sequence_number}, retry {retries}/3")
                        except Exception as e:
                            self.logger.error(f"ACK ERROR: {e}, addr: {addr if 'addr' in locals() else 'N/A'}")
                ...
                if not ack_received:
                    self.logger.error(f"Failed to receive ACK for seq: {self.sequence_number} after 3 retries")

                elapsed = time.time() - start_time
                time.sleep(max(0, target_period - elapsed))
        except Exception as e:
            self.logger.error(f"send_state crashed: {e}")
        finally:
            print("send_state exited")


    def receive_messages(self):
        # self.received_state = None
        self.logger.info(f"Starting receive thread for V{self.vehicle_id}")
        while self.running.is_set():
            # print("receive_messages running")
            target_period = 0.1
            start_time = time.time()
            try:
                # print("Receive loop running")
                self.logger.debug(f"Receive loop running for V{self.vehicle_id}, self.running: {self.running}")
                while time.time() - start_time < 0.09 and self.running.is_set():
                    try:
                        print("rec1")
                        print("ack received",self.heartbeat)
                        ...
                        if msg_type == 'state' and incoming.get('id') != self.vehicle_id:
                            ...
                            try:
                                ...
                                self.logger.error(f"ACK SEND ERROR: {e}, addr: {addr}")
                                
                            # Update follower's leader_state directly
                            if not self.vehicle.is_leader:
                                with self.lock:
                                    print("rec3")
                                    self.vehicle.leader_state = incoming
                                    print("[receiver]leader_state",self.vehicle.leader_state)
                                    # self.received_state = incoming
                                    self.logger.info(f"Updated leader_state: {incoming}")
                        ...
                        self.logger.error(f"Receive socket error: {e}")
                        break
            except Exception as e:
