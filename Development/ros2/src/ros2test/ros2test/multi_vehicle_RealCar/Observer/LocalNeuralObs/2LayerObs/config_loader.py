...
def flatten_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    flat['inference_only'] = learn.get('inference_only', False)
    flat['online_learning_enabled'] = learn.get('online_learning_enabled', True)
    flat['online_update_interval'] = learn.get('online_update_interval', 1)
    flat['online_warmup_steps'] = learn.get('online_warmup_steps', 0)
    flat['online_learning_min_speed'] = learn.get('online_learning_min_speed', 0.0)
    flat['online_learning_max_speed'] = learn.get('online_learning_max_speed', float('inf'))
    flat['online_learning_requires_gps'] = learn.get('online_learning_requires_gps', True)
    flat['online_learning_min_steering'] = learn.get('online_learning_min_steering', 0.0)
    flat['online_learning_min_yaw_rate'] = learn.get('online_learning_min_yaw_rate', 0.0)
