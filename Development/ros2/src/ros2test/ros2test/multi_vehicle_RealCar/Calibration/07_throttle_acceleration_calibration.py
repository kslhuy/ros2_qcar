...
def _analyse_transition(
    ...
    if np.isfinite(a0_model) and abs(a0_model) > 1e-6:
        acc_to_throttle_model = float(delta_u / a0_model)
    else:
        acc_to_throttle_model = float("nan")

    if np.isfinite(a_peak_meas) and abs(a_peak_meas) > 1e-6:
        acc_to_throttle_peak = float(delta_u / a_peak_meas)
    else:
        acc_to_throttle_peak = float("nan")
    ...
    return {
        ...
        "acc_to_throttle_model": acc_to_throttle_model,
        "acc_to_throttle_peak": acc_to_throttle_peak,
        "t63_s": t63,
        "t90_s": t90,
        "t95_s": t95,
    ...
    with (
        ...
        for idx, (u_from, u_to) in enumerate(transitions, start=1):
            ...
            lut_log.write(
                ...
                info["acc_to_throttle_model"],
                info["acc_to_throttle_peak"],
                info["t63_s"],
                info["t90_s"],
                info["t95_s"],
                info["lead_time_s"],
            ...
            print(
                ...
                f"a_peak={info['a_peak_meas_mps2']:.4f} m/s^2\n"
                f"  gain_model={info['acc_to_throttle_model']:.4f}  "
                f"gain_peak={info['acc_to_throttle_peak']:.4f}  "
...
def print_summary(results: List[Dict[str, object]]):
    ...
    print(
        ...
        f"{'a0[m/s2]':>10}  {'gain_mdl':>8}  {'gain_pk':>8}"
    )
    print(f"  {'-' * 13}  {'-' * 15}  {'-' * 8}  {'-' * 9}  {'-' * 10}  {'-' * 8}  {'-' * 8}")
    ...
    for row in results:
        ...
        print(
            ...
            f"{row['a0_model_mps2']:10.4f}  {row['acc_to_throttle_model']:8.4f}  {row['acc_to_throttle_peak']:8.4f}"
        )
