"""
Vehicle-side ZMQ client for passive online calibration.

Lightweight transport layer (mirrors OnlineSysIDZMQClient):
- PUB socket for streaming calibration samples to external worker
- PUB socket for control commands (start/stop/analyse/clear)
- SUB socket for receiving status/results from worker

Does NOT run analysis locally — that is handled by the worker process.
"""
...
class OnlineCalibrationZMQClient:
    ...
    def __init__(
        ...
        logger=None,
        vehicle_id: int = 0,
        sample_port: int = 18890,
        control_port: int = 18891,
        status_host: str = "127.0.0.1",
        status_port: int = 18892,
        bind_ip: str = "*",
    ):
        self.logger = logger
    ...
    def start(self) -> bool:
        ...
        if not ZMQ_AVAILABLE:
            ...
            self._log_warn("[OnlineCal-ZMQ] pyzmq not available")
            return False
        ...
        try:
            ...
            self._status_sub.connect(
                f"tcp://{self.status_host}:{self.status_port}"
            )
            ...
            self._last_error = ""
            self._log_info(
                f"[OnlineCal-ZMQ] Started: "
                f"sample=tcp://{self.bind_ip}:{self.sample_port}, "
            ...
            self._log_error("[OnlineCal-ZMQ] Failed to start", exc)
            self.stop()
            return False
    ...
    def stop(self) -> None:
        self._collecting = False
        self._running = False
        for sock in (self._sample_pub, self._control_pub, self._status_sub):
            if sock is not None:
                try:
                    sock.close(linger=0)
                except Exception:
