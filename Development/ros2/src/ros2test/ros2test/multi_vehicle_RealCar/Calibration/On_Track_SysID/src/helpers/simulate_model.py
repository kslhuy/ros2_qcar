...
class Simulator:
  def __init__(self, model_name, package_path=None):
    _, self.tiretype = model_name.split("_")
    self.model = get_dotdict(model_name, package_path=package_path)
    self.sol = None
...
class LookupGenerator:
  def __init__(self, racecar_version, save_LUT_name, package_path=None):
    self.racecar_version = racecar_version
    model_name = self.racecar_version +"_pacejka"
    self.package_path = package_path
    self.sim = Simulator(model_name, package_path=package_path)
  ...
  def run_generator(self):
    ...
    if PLOT_LOOKUP:
      LOGGER.logwarn("Lookup Table has been generated. Close the plot window (press Q) to save the lookup table.")
      self.plot_lookup()
    self.save_lookup()

  def load_lookup(self, model, name):
    package_root = get_package_path(package_path=self.package_path)
    file_path = os.path.join(package_root, "models", model, name + "_lookup_table.csv")
  ...
  def save_lookup(self):
    package_root = get_package_path(package_path=self.package_path)
    file_path = os.path.join(package_root, "models", self.racecar_version, self.save_LUT_name + "_lookup_table.csv")
    np.savetxt(file_path, self.lookup_table, delimiter=",")
    LOGGER.loginfo(f"SAVED LOOKUP TABLE TO: {file_path}")
