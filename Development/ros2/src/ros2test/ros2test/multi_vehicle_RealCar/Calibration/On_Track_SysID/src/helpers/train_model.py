...
from helpers.runtime import get_logger, get_package_path
...
def simulated_data_gen(nn_model, model, avg_vel, dt=0.02, timesteps=500):
    ...
    delta = np.linspace(-0.4, 0.4, timesteps)
...
def get_model_param(racecar_version, package_path=None):
    """
    ...
    yaml_file = os.path.join(package_root, "params/pacejka_params.yaml")
    with open(yaml_file, "r") as file:
        pacejka_params = yaml.safe_load(file)

    # Load vehicle parameters
    yaml_file = os.path.join(
        package_root, "models", racecar_version, racecar_version + "_pacejka.txt"
    )
    with open(yaml_file, "r") as file:
...
def get_nn_params(package_path=None):
    """
    ...
    """
    package_root = get_package_path(package_path=package_path)
    yaml_file = os.path.join(package_root, "params/nn_params.yaml")
    with open(yaml_file, "r") as file:
...
def generate_training_set(training_data, model, dt=0.02):
    """
...
def nn_train(
    ...
    generate_lut=True,
    package_path=None,
    always_plot_last_iteration=True,
):
    """
    ...
    model = get_model_param(racecar_version, package_path=package_path)
    nn_params = get_nn_params(package_path=package_path)
    ...
    np.savetxt(
        ...
        training_data_raw,
        delimiter=",",
        header="v_x,v_y,omega,delta",
        comments="",
    )
    LOGGER.loginfo(f"Saved raw training data to {csv_path}")
    ...
    for i in range(1, num_of_iterations + 1):
        ...
        X_train, y_train = generate_training_set(training_data, model, dt=dt)
    ...
    car_model = get_dotdict(model_name, package_path=package_path)
    car_model.C_Pf = C_Pf_identified
    car_model.C_Pr = C_Pr_identified
    save(car_model, package_path=package_path)

    if generate_lut:
        from helpers.simulate_model import LookupGenerator

        LOGGER.loginfo("LUT is being generated...")
        LookupGenerator(
            racecar_version, save_LUT_name, package_path=package_path
        ).run_generator()
