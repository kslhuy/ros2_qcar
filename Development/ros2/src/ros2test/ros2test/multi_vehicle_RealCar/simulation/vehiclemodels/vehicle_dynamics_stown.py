...
def vehicle_dynamics_st_delayed(x, uInit, p, type):
    ...
    # ensure velocity is not exactly zero to avoid division by zero
    v_x4 = x[4] if abs(x[4]) > 1e-5 else (1e-5 if x[4] >= 0 else -1e-5)
    alpha_f = -math.atan((x[5] + x[6] * lf) / v_x4) + x[2] 
    alpha_r = -math.atan((x[5] - x[6] * lr) / v_x4)
...
def vehicle_dynamics_st(x, uInit, p, type):
    ...
    # compute lateral tire slip angles
    # ensure velocity is not exactly zero to avoid division by zero
    v_x3 = x[3] if abs(x[3]) > 1e-5 else (1e-5 if x[3] >= 0 else -1e-5)
    alpha_f = -math.atan((x[4] + x[5] * lf) / v_x3) + u[0] 
    alpha_r = -math.atan((x[4] - x[5] * lr) / v_x3)
