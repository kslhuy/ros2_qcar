...
class PP_Controller:
    ...
    def update_params(self, params: dict):
        ...
        if self.logger_info:
            self.logger_info(f"[PP_Controller] Updated params: {params}")
    ...
    def main_loop(
        ...
        if L1_point is not None and np.isfinite(L1_point).all():
            steering_angle = self.calc_steering_angle(
                L1_point, L1_distance, yaw, lat_e_norm, v
            )
        else:
    ...
    def calc_steering_angle(self, L1_point, L1_distance, yaw, lat_e_norm, v):
        ...
        if self.state == "TRAILING" and (self.opponent is not None):
            ...
            la_position_steer = [
                self.position_in_map[0, 0] + v[0] * adv_ts_st,
                self.position_in_map[0, 1] + v[1] * adv_ts_st,
            ]
            idx_la_steer = self.nearest_waypoint(
                la_position_steer, self.waypoint_array_in_map[:, :2]
            )
        ...
        L1_vector = np.array(
            [
                L1_point[0] - self.position_in_map[0, 0],
                L1_point[1] - self.position_in_map[0, 1],
            ]
        )
        if np.linalg.norm(L1_vector) == 0:
            ...
            eta = np.arcsin(
                np.dot([-np.sin(yaw), np.cos(yaw)], L1_vector)
        ...
        threshold = 0.5  # Increase threshold to prevent log spam from simulator noise
        if abs(steering_angle - self.curr_steering_angle) > threshold:
            self.logger_warn(
                f"[PP Controller] steering angle clipped (jumped by {abs(steering_angle - self.curr_steering_angle):.2f})"
            )
    ...
    def calc_L1_point(self, lateral_error):
        ...
        if len(self.waypoint_array_in_map[self.idx_nearest_waypoint :]) > 2:
            # Estimate local curvature around the current segment instead of averaging
            # to the end of the route, which can mask near-term turns.
            curv_window = 50
            end_idx = min(
                self.idx_nearest_waypoint + curv_window,
                self.waypoint_array_in_map.shape[0],
            )
            curv_slice = self.waypoint_array_in_map[
                self.idx_nearest_waypoint : end_idx, 5
            ]
            self.curvature_waypoints = float(np.mean(np.abs(curv_slice)))
    ...
    def calc_speed_command(self, v, lat_e_norm):
        ...
        la_position = [
            self.position_in_map[0, 0] + v[0] * adv_ts_sp,
            self.position_in_map[0, 1] + v[1] * adv_ts_sp,
        ]
        idx_la_position = self.nearest_waypoint(
            la_position, self.waypoint_array_in_map[:, :2]
        )
        global_speed = self.waypoint_array_in_map[idx_la_position, 2]
        if self.state == "TRAILING" and (
    ...
    def trailing_controller(self, global_speed):
        ...
        self.gap = (
            self.opponent[0] - self.position_in_map_frenet[0]
        ) % self.track_length  # gap to opponent
        ...
        self.trailing_command = np.clip(
            self.opponent[2] - p_value - i_value - d_value, 0, global_speed
        )
        if not self.opponent[4] and self.gap_actual > self.gap_should:
            self.trailing_command = max(
                self.blind_trailing_speed, self.trailing_command
            )
    ...
    def speed_steer_scaling(self, steer, speed):
        ...
        factor = (
            1
            - np.clip((speed - self.start_scale_speed) / (speed_diff), 0.0, 1.0)
    ...
    def speed_adjust_lat_err(self, global_speed, lat_e_norm):
        ...
        """
        # Scale down speed with lateral error and local path curvature.
        # Keep a baseline reduction on straights when d-error grows, and
        # increase reduction in tighter turns.
        lat_e_coeff = np.clip(self.lat_err_coeff, 0.0, 1.0)
        lat_term = np.clip(lat_e_norm * 2.0, 0.0, 1.0)

        curv_ref = 0.25
        curv_norm = np.clip(float(self.curvature_waypoints) / curv_ref, 0.0, 1.0)
        curv_weight = 0.30 + 0.70 * curv_norm

        speed_scale = 1.0 - lat_e_coeff * lat_term * curv_weight
        global_speed *= np.clip(speed_scale, 0.40, 1.0)
        return global_speed
    ...
    def speed_adjust_heading(self, speed_command):
        ...
        if heading_error < np.pi / 9:  # 20 degrees error is okay
            ...
            scaler = 0.5
        self.logger_info(
            f"[MAP Controller] heading error decreasing velocity by {scaler}"
        )
    ...
    def waypoint_at_distance_before_car(
        self, distance, waypoints, idx_waypoint_behind_car
    ):
        """
