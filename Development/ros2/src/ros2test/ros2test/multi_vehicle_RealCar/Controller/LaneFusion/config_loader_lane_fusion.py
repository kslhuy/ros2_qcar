"""
Lane Fusion Config Loader

Loads lane fusion configuration from YAML file.
"""
...
DEFAULT_CONFIG_FILE = os.path.join(CONFIG_DIR, 'config_lane_fusion.yaml')
...
class LaneFusionConfigLoader:
    """
    Loads and manages lane fusion configuration from YAML file.
    
    Usage:
        config = LaneFusionConfigLoader()
        fusion_settings = config.get_fusion_settings()
    ...
    def _load_config(self):
        ...
        if os.path.exists(self.config_file):
            ...
            print(f"[LaneFusion] Config file not found: {self.config_file}, using defaults")
            self._config = {}
...
_config_instance: Optional[LaneFusionConfigLoader] = None


def get_lane_fusion_config(config_file: str = None) -> LaneFusionConfigLoader:
    """
    Get the lane fusion config loader instance.
    
    Args:
        config_file: Optional path to config file
        
    Returns:
        LaneFusionConfigLoader instance
    """
    global _config_instance
    if _config_instance is None or config_file is not None:
        _config_instance = LaneFusionConfigLoader(config_file)
