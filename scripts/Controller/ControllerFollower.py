...
class ControllerFollower:
    ...
    def step(self):
        ...
        self.trust_model.v_score_log.append(v_score)

        # NEW: distance score
        d_score = self.trust_model.evaluate_distance(distance, self.default_safe_distance)
        self.trust_model.d_score_log.append(d_score)

        # NEW: gamma cross + sudden monitor
        gamma_cross = abs(v_follower - v_leader)
        D_pos = distance
        D_vel = abs(v_follower - v_leader)

        self.trust_model.gamma_cross_log.append(gamma_cross)
        self.trust_model.monitor_sudden(gamma_cross, D_pos, D_vel)
